...
const TV_API_TOKEN = process.env.REACT_APP_TV_API_TOKEN || "hurricane-tv-2024-secret";
const TV_HEADERS = { "Content-Type": "application/json", "x-api-token": TV_API_TOKEN };
                  ...
                  <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
                    <div className="card" style={{ background:"rgba(255,0,255,.1)", border:"2px solid rgba(255,0,255,.5)", textAlign:"center" }}>
                      ...
                      <button className="btn btn-p" style={{ padding:"14px 32px", fontSize:"1rem", width:"100%", letterSpacing:1 }}
                        onClick={() => {
                          Promise.all([
                            fetch("http://192.168.3.13:3500/api/tv/9/launch-url", { method:"POST", headers:TV_HEADERS, body:JSON.stringify({ url:"https://karaoke-coral-seven.vercel.app/?modo=tv" }) }),
                            fetch("http://192.168.3.13:3500/api/tv/8/launch-url", { method:"POST", headers:TV_HEADERS, body:JSON.stringify({ url:"https://karaoke-coral-seven.vercel.app/?modo=nextup" }) })
                    ...
                    <div className="card">
                      ...
                      <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                        <button className="btn btn-p" style={{ padding:"10px 18px", fontSize:".85rem" }} onClick={() => fetch("http://192.168.3.13:3500/api/tv/9/launch-url", { method:"POST", headers:TV_HEADERS, body:JSON.stringify({ url:"https://karaoke-coral-seven.vercel.app/?modo=tv" }) }).then(() => toast$("🎤 Karaoke Screen opened!")).catch(() => toast$("❌ Error", "err"))}>🎤 Karaoke Screen</button>
                        ...
                        <button className="btn btn-p" style={{ padding:"10px 18px", fontSize:".85rem", background:"linear-gradient(135deg,#555,#333)" }} onClick={() => fetch("http://192.168.3.13:3500/api/tv/9/command", { method:"POST", headers:TV_HEADERS, body:JSON.stringify({ command:"Home" }) }).then(() => toast$("✅ Home sent!")).catch(() => toast$("❌ Error", "err"))}>🏠 Home</button>
                      </div>
                    </div>
                    ...
                    <div className="card">
                      ...
                      <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                        ...
                        <button className="btn btn-p" style={{ padding:"10px 18px", fontSize:".85rem", background:"linear-gradient(135deg,#555,#333)" }} onClick={() => fetch("http://192.168.3.13:3500/api/tv/8/command", { method:"POST", headers:TV_HEADERS, body:JSON.stringify({ command:"Home" }) }).then(() => toast$("✅ Home sent!")).catch(() => toast$("❌ Error", "err"))}>🏠 Home</button>
                      </div>
                    </div>
