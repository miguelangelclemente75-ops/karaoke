...
export default function KaraokeApp() {
  ...
  const confirmRequest = () => {
    const entry = { ...pendingSong, singer: singerName, table: selectedTable, id: Date.now() };
    setQueue(q => [...q, entry]);
  ...
  const removeFromQueue = (id) => {
    ...
    showToast("Canción removida", "error");
  };
  ...
  return (
    <div style={{
      ...
      {notes.map((n, i) => (
        <div key={i} className="floating-note" style={{ left: n.left, animationDelay: n.animationDelay, fontSize: n.fontSize, opacity: n.opacity }}>♪</div>
      ))}

      {toast && (
        <div style={{ position:"fixed",top:20,right:20,zIndex:1000,background:toast.type==="error"?"rgba(255,50,50,0.9)":"rgba(50,200,100,0.9)",padding:"12px 24px",borderRadius:16,fontWeight:"bold",animation:"toastIn 0.3s ease",boxShadow:"0 4px 20px rgba(0,0,0,0.3)" }}>{toast.msg}</div>
      )}
        ...
        <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:500,padding:20 }}>
          <div className="card" style={{ maxWidth:380,width:"100%",textAlign:"center" }}>
            ...
            <p style={{ color:"rgba(255,255,255,0.7)",margin:"0 0 20px" }}>
              <strong style={{ color:"#fff" }}>{pendingSong.title}</strong><br/>{pendingSong.artist} • Mesa {selectedTable}
            </p>
        ...
        <div style={{ display:"flex",gap:8,justifyContent:"center",flexWrap:"wrap" }}>
          {["home","songs","queue","admin"].map(v => (
            <button key={v} className={`tab-btn ${view===v?"active":""}`} onClick={() => setView(v)}>
              {v==="home"?"🏠 Inicio":v==="songs"?"🎵 Canciones":v==="queue"?`📋 Cola (${queue.length})`:"🎛️ Admin"}
            </button>
          ))}
        </div>
      </div>
      ...
      <div style={{ position:"relative",zIndex:10,padding:"24px 16px",maxWidth:700,margin:"0 auto" }}>
          ...
          <div style={{ animation:"slideIn 0.3s ease" }}>
            ...
            {selectedTable && singerName && (
              <div style={{ textAlign:"center",animation:"slideIn 0.3s ease" }}>
                <p style={{ color:"rgba(255,255,255,0.6)",marginBottom:16 }}>¡Listo, <strong style={{ color:"#ff88ff" }}>{singerName}</strong>! Mesa <strong style={{ color:"#ff88ff" }}>{selectedTable}</strong> seleccionada.</p>
        ...
        {view==="songs" && (
          <div style={{ animation:"slideIn 0.3s ease" }}>
            {!selectedTable && (
              <div className="card" style={{ textAlign:"center",marginBottom:16,borderColor:"rgba(255,150,0,0.4)" }}>
                ⚠️ Selecciona tu mesa en <strong style={{ color:"#ff88ff",cursor:"pointer" }} onClick={() => setView("home")}>Inicio</strong> primero
              </div>
            )}
                ...
                {queue.map((entry,i) => (
                  <div key={entry.id} className="card" style={{ padding:"16px 20px" }}>
                    <div style={{ display:"flex",alignItems:"center",gap:12 }}>
                      <div style={{ width:36,height:36,borderRadius:"50%",background:i===0?"linear-gradient(135deg,#ff00ff,#9900ff)":"rgba(255,255,255,0.1)",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:"bold",fontSize:"0.9rem",flexShrink:0 }}>{i+1}</div>
        ...
        {view==="admin" && (
          <div style={{ animation:"slideIn 0.3s ease" }}>
            <h2 style={{ margin:"0 0 16px",color:"#ff88ff" }}>🎛️ Panel del DJ</h2>
            <div style={{ display:"flex",gap:8,marginBottom:20 }}>
              {["queue","now"].map(v => (
                <button key={v} className={`tab-btn ${adminView===v?"active":""}`} onClick={() => setAdminView(v)}>
                  {v==="queue"?`📋 Cola (${queue.length})`:"🎤 En vivo"}
                </button>
              ))}
            </div>
            ...
            {adminView==="queue" && (
              <div>
                {queue.length===0 ? (
                  <div className="card" style={{ textAlign:"center",padding:40 }}>
                    <p style={{ color:"rgba(255,255,255,0.5)" }}>Cola vacía</p>
                  </div>
                ) : (
                  <div style={{ display:"flex",flexDirection:"column",gap:10 }}>
                    {queue.map((entry,i) => (
              ...
              <div>
                {!currentSong ? (
                  ...
                  <div className="card" style={{ textAlign:"center" }}>
                    <div style={{ fontSize:"4rem",marginBottom:12,animation:isPlaying?"pulse 1s ease infinite":"none" }}>{currentSong.emoji}</div>
                    ...
                    {score!==null && (
                      <div style={{ animation:"slideIn 0.5s ease" }}>
                        ...
                        {queue.length>0 && (
                          <button className="btn-primary" style={{ marginTop:16 }} onClick={() => startSong(queue[0])}>▶ Siguiente canción</button>
                        )}
                      </div>
                    )}
                  </div>
