...
/**
 * NOTAS IMPORTANTES
 * 1) Mueve estas credenciales a variables de entorno.
 * 2) NO uses contraseña de admin hardcodeada en producción.
 * 3) Protege /pending, /queue y /current con Firebase Auth + Rules.
 */
...
export default function App() {
  ...
  const submitReq = async () => {
    ...
    if (!table) return toast$("Please select your table", "err");
      ...
      {toast && (
        <div
          style={{
            ...
            zIndex: 1100,
            background:
              toast.type === "err" ? "rgba(220,40,40,.92)" : "rgba(30,180,90,.92)",
            padding: "11px 22px",
            borderRadius: 14,
            fontWeight: "bold",
            fontSize: ".9rem",
                    ...
                    {TABLES.map((t) => (
                      <button
                        ...
                        style={{
                          ...
                          borderRadius: 13,
                          border:
                            table === t ? "2px solid #ff00ff" : "1px solid rgba(255,255,255,0.18)",
            ...
            {name && table && (
              <div style={{ textAlign: "center", animation: "slideIn .3s ease" }}>
                <p style={{ color: "rgba(255,255,255,0.55)", marginBottom: 14, fontSize: ".9rem" }}>
                  ...
                  <strong style={{ color: "#ff88ff" }}>{table}</strong>
                </p>
                  ...
                  {ytResults.map((v) => (
                    <div key={v.id} className={`vcard ${pickedVideo?.id === v.id ? "sel" : ""}`} onClick={() => setPickedVideo(v)}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontWeight: "bold", fontSize: ".82rem", lineHeight: 1.3, marginBottom: 3, overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>
                          {v.title}
                        </div>
                      ...
                      {pickedVideo?.id === v.id ? (
                        ...
                        <span style={{ fontSize: ".75rem", color: "rgba(255,255,255,0.35)", flexShrink: 0 }}>Select</span>
                      )}
                    </div>
                  ))}
                </div>
                  ...
                  return (
                    <div key={req.fbKey || i} className="card" style={{ padding: "14px 18px" }}>
                      <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 10 }}>
                        {!!req.thumb && (
                          <img src={req.thumb} alt="" style={{ width: 72, height: 40, borderRadius: 7, objectFit: "cover", flexShrink: 0 }} onError={(e) => (e.currentTarget.style.display = "none")} />
                        )}
                        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ animation: "slideIn .3s ease" }}>
            ...
            {currentSong && (
              <div className="card-green" style={{ marginBottom: 14 }}>
                <p style={{ margin: "0 0 6px", fontSize: ".72rem", color: "rgba(255,255,255,0.5)" }}>
                  <span className="live-dot" />NOW PLAYING ON TV
                </p>
                <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                  {!!currentSong.thumb && (
                    <img src={currentSong.thumb} alt="" style={{ width: 64, height: 36, borderRadius: 7, objectFit: "cover" }} onError={(e) => (e.currentTarget.style.display = "none")} />
                  )}
                  <div>
                ...
                {queue.map((entry, i) => (
                  <div key={entry.fbKey} className="card" style={{ padding: "12px 16px" }}>
                    <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                      ...
                      {!!entry.thumb && (
                        <img src={entry.thumb} alt="" style={{ width: 70, height: 40, borderRadius: 7, objectFit: "cover", flexShrink: 0 }} onError={(e) => (e.currentTarget.style.display = "none")} />
                      )}
                      <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      pending.map((req) => (
                        <div key={req.fbKey} className="card">
                          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                            {!!req.thumb && (
                              <img src={req.thumb} alt="" style={{ width: 90, height: 50, borderRadius: 8, objectFit: "cover" }} onError={(e) => (e.currentTarget.style.display = "none")} />
                            )}
                            <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      queue.map((entry, index) => (
                        <div key={entry.fbKey} className="card">
                          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                            ...
                            {!!entry.thumb && (
                              <img src={entry.thumb} alt="" style={{ width: 90, height: 50, borderRadius: 8, objectFit: "cover" }} onError={(e) => (e.currentTarget.style.display = "none")} />
                            )}
                            <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                            <button className="btn btn-p" onClick={() => playSong(entry)} disabled={workingId === entry.fbKey}>Play</button>
